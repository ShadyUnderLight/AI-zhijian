...
struct NodeConfigSheet: View {
    ...
    private var promptTemplateConfig: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            if case .promptTemplate = config {
                ...
                let connectedNames = definition.connectedVariableNames(for: node.id)
                let badVars = referenced.subtracting(connectedNames)
                ...
                if !badVars.isEmpty {
                    HStack(spacing: 4) {
                        ...
                        Text("以下变量未连接，运行时不会替换: \(badVars.sorted().joined(separator: ", "))")
                            .font(.caption2)
                            .foregroundColor(.orange)
                    }
                    .padding(6)
                ...
                if !vars.isEmpty {
                    VStack(alignment: .leading, spacing: 4) {
                        Text("变量面板")
                            .font(.caption)
                            .foregroundColor(.secondary)
                        VariablePanelView(variables: vars) { variable in
                            let name = variable.variableName ?? variable.portName
                            let current = promptTemplateText.wrappedValue
                            promptTemplateText.wrappedValue = current + "{{\(name)}}"
                        }
                        .padding(8)
    ...
    private func templatePreviewText(_ template: String, variables: [UpstreamVariable]) -> some View {
        let connectedNames = Set(variables.compactMap(\.variableName))
