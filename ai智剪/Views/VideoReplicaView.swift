...
struct VideoReplicaView: View {
    ...
    @State private var targetStyle = "同风格"
    ...
    private let resolutionOptions = ["720p", "1080p", "2k", "4k"]
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 14) {
                ...
                // 目标风格
                VStack(alignment: .leading, spacing: 6) {
                    Text("目标风格").font(.headline)
                    TextField("例如: 同风格、电影感、动漫风", text: $targetStyle)
                ...
                // 时长
                VStack(alignment: .leading, spacing: 6) {
                    Text("生成时长: \(duration) 秒").font(.headline)
                    Stepper(value: $duration, in: 5...60) {
                        Text("\(duration) 秒")
                            .font(.body)
                    }
                    .frame(maxWidth: 200)
                ...
                HStack {
                    Button(action: startGeneration) {
                        if isTaskPending {
                            ...
                            Label("开始复刻", systemImage: "square.on.square")
                        }
                    }
                    .buttonStyle(.borderedProminent)
                    .disabled(videoData == nil || videoName.isEmpty || isTaskPending || preflight.state.isBlocking)
                }
        ...
        .onChange(of: targetStyle) { _, _ in triggerPreflight() }
    ...
    private func applyEditIfNeeded() {
        ...
        guard case .videoReplica(let p) = item.params else { return }
    ...
    private func applyRecordIfNeeded() {
        ...
        guard let snapshot = record.paramsSnapshot,
              ...
              case .videoReplica(let t, let d, let r) = params
        else { return }
        targetStyle = t
        duration = d
        resolution = r
    ...
    private func startGeneration() {
        ...
        guard !targetStyle.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else {
            errorMessage = "请输入目标风格"
            return
        }
        errorMessage = nil
        resultVideoUrl = nil
        ...
        let params = VideoReplicaJobParams(
            videoData: vData, videoName: videoName, videoMime: videoMime,
            targetStyle: targetStyle, duration: duration, resolution: resolution
        )
        let item = GenerationQueueItem(kind: .videoReplica, createdAt: Date(), params: .videoReplica(params))
    ...
    private var presetRow: some View {
        let kind = PresetKind.videoReplica
        ...
        return HStack(spacing: 8) {
            ...
            if available.isEmpty {
                ...
                .onChange(of: selectedPresetId) { _, id in
                    ...
                    guard case .videoReplica(let p) = preset.params else { return }
                    targetStyle = p.targetStyle
                    duration = p.duration
                    resolution = p.resolution
                }
        ...
        .alert("保存预设", isPresented: $showSavePresetAlert) {
            ...
            Button("保存") {
                ...
                let params = VideoReplicaPresetParams(targetStyle: targetStyle, duration: duration, resolution: resolution)
                presetStore.save(name: name, params: .videoReplica(params))
