...
struct CharacterReplaceView: View {
    ...
    @State private var selectedRefURL: URL?
    @State private var refImageData: Data?
    @State private var refImageName = ""
    @State private var similarity: Double = 0.8
    @State private var faceFidelity: Double = 0.9
    ...
    @State private var isLoadingRef = false
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 14) {
                ...
                // 参考人物图片选择
                VStack(alignment: .leading, spacing: 6) {
                    Text("参考人物图片").font(.headline)
                    filePickerRow(
                        url: $selectedRefURL, data: $refImageData, name: $refImageName,
                        isLoading: $isLoadingRef, label: "选择人物图片",
                ...
                VStack(alignment: .leading, spacing: 8) {
                    VStack(alignment: .leading, spacing: 2) {
                        HStack {
                            Text("相似度").font(.caption2).foregroundColor(.secondary)
                            Spacer()
                    ...
                    VStack(alignment: .leading, spacing: 2) {
                        HStack {
                            Text("面部保真度").font(.caption2).foregroundColor(.secondary)
                            Spacer()
                            Text(String(format: "%.2f", faceFidelity))
                                .font(.caption2).foregroundColor(.secondary).monospacedDigit()
                        }
                        Slider(value: $faceFidelity, in: 0...1, step: 0.05)
                            .disabled(isTaskPending)
                    }
                }
                ...
                HStack {
                    Button(action: startGeneration) {
                        if isTaskPending {
                            ...
                            Label("开始替换", systemImage: "person.crop.circle.badge.plus")
                        }
                    }
                    .buttonStyle(.borderedProminent)
                    .disabled(videoData == nil || refImageData == nil || isTaskPending || preflight.state.isBlocking)
                }
        ...
        .onChange(of: refImageData?.count) { _, _ in triggerPreflight() }
        .onChange(of: similarity) { _, _ in triggerPreflight() }
        .onChange(of: faceFidelity) { _, _ in triggerPreflight() }
    ...
    private func applyEditIfNeeded() {
        ...
        guard case .characterReplace(let p) = item.params else { return }
        videoData = p.videoData
        videoName = p.videoName
        refImageData = p.referenceImageData
        refImageName = p.referenceImageName
    ...
    private func applyRecordIfNeeded() {
        ...
        guard let snapshot = record.paramsSnapshot,
              ...
              case .characterReplace(let m) = params
        else { return }
        similarity = m.similarity
        faceFidelity = m.faceFidelity
        errorMessage = nil
        resultVideoUrl = nil
        isTaskPending = false
        submittedTaskId = nil
    ...
    private func startGeneration() {
        ...
        guard let refData = refImageData, !refData.isEmpty else {
            errorMessage = "请先选择参考人物图片"
            return
        }
        errorMessage = nil
        resultVideoUrl = nil
        ...
        let refMime = mimeType(for: refImageName, defaultMime: "image/png")
        let params = CharacterReplaceJobParams(
            videoData: vData, videoName: videoName, videoMime: videoMime,
            referenceImageData: refData, referenceImageName: refImageName,
            referenceImageMime: refMime, similarity: similarity, faceFidelity: faceFidelity
        )
        let item = GenerationQueueItem(kind: .characterReplace, createdAt: Date(), params: .characterReplace(params))
    ...
    private var presetRow: some View {
        let kind = PresetKind.characterReplace
        ...
        return HStack(spacing: 8) {
            ...
            if available.isEmpty {
                ...
                .onChange(of: selectedPresetId) { _, id in
                    ...
                    guard case .characterReplace(let p) = preset.params else { return }
                    similarity = p.similarity
                    faceFidelity = p.faceFidelity
                }
            }
        ...
        .alert("保存预设", isPresented: $showSavePresetAlert) {
            ...
            Button("保存") {
                ...
                let params = CharacterReplacePresetParams(similarity: similarity, faceFidelity: faceFidelity)
                presetStore.save(name: name, params: .characterReplace(params))
    ...
    private func triggerPreflight() {
        guard videoData != nil, refImageData != nil else { preflight.reset(); return }
        let params = CharacterReplaceJobParams(
            videoData: videoData ?? Data(), videoName: videoName, videoMime: mimeType(for: videoName, defaultMime: "video/mp4"),
            referenceImageData: refImageData ?? Data(), referenceImageName: refImageName,
            referenceImageMime: mimeType(for: refImageName, defaultMime: "image/png"),
            similarity: similarity, faceFidelity: faceFidelity
        )
        preflight.schedule(for: .characterReplace(params))
    }
