...
final class WorkflowCanvasTests: XCTestCase {
    ...
    func testRetryConfigChangePreservesUnchangedUpstream() {
        ...
        for nid in nodeIds {
            store.runState.nodeStatuses[nid] = .succeeded
            store.runState.cachedNodeOutputs[nid] = ["out": .text("cached")]
        }
